...
# 用法: build_lambda_zip.sh <service> <target>
#   target=aws:   业务代码 + 跨平台 wheel 依赖（linux/x86_64/py3.12）给 AWS Lambda runtime
#   target=local: 仅业务代码，依赖交给 MiniStack 自定义镜像预装（含 psycopg2/boto3/langfuse）
# 本地打 x86 wheel 在 Apple Silicon MiniStack Warm Worker 会因 .so 架构不匹配起不来。
set -euo pipefail
...
ZIP_FILE="${OUT_DIR}/${SERVICE_NAME}-${TARGET}.zip"
rm -f "$ZIP_FILE"
